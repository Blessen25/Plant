...
export const Button_with_icon_primary:React.FC<ButtonProps> = (props) => {
    ...
    return(
        <>
            <a href={`${props.route ? props.route : '#'}`} style={props.styles} className={`buttonclassprimary button_icon text-normal ${props.className ? props.className : ''}`}><span>{<i className={`${props.icon} icon-md`}></i>}</span>{props.text}</a>
        </>
    )
